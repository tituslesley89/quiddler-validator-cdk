...
class DictionaryClient:
    def __init__(self, bucket_name):
        ...
        self.api_key='b20cb889-918c-40a4-8250-44ee3669b574'
    ...
    def toggle_exception_word(self, access_key, word):
        if word in self.access_keys_set:
            if word in self.house_words_set:
                self.house_words_set.remove(word)
            else:
    ...
    def toggle_exception_subject(self, access_key, subject):
        if word in self.access_keys_set:
            if subject in self.invalid_subjects_set:
                self.invalid_subjects_set.remove(subject)
            else:
